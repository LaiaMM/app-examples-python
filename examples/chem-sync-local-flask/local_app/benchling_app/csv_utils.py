...
def download_csv(app: App, entit_id: str, destination_dict)-> None: # , destination_path: Path) -> None:
    """
    ...
    blob_file_name = benchling_csv_ent.name

    # base, version = blob_file_name.split("_", 1)  # split on first underscore
    # config = destination_dict.get(base)
    config = destination_dict.get(blob_file_name)
    if config:
